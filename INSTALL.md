...
```bash
sudo apt-get install libjpeg-dev libtiff5-dev libpng-dev libfreetype-dev libgif-dev \
libgtk-3-dev libxml2-dev libpango1.0-dev libcairo2-dev \
libspiro-dev libwoff-dev python3-dev ninja-build \
...
If you want to do auto-tracing around character images you should also download either:

* Peter Selinger's [`potrace`](https://potrace.sf.net/)
* Martin Weber's [`autotrace` program](https://sourceforge.net/projects/autotrace/)
...
- Image Libraries (to allow FontForge to import images in those formats generally used as backgrounds for autotracing)

* [`libpng`](https://www.libpng.org/pub/png/libpng.html) (and required helper [`zlib`](https://www.zlib.net/))

* [`libtiff`](https://www.libtiff.org/)
* [`libungif`](https://gnuwin32.sourceforge.net/packages/libungif.htm)
* [`libjpeg`](https://www.ijg.org/)
* [`libxml2`](https://xmlsoft.org/) To parse SVG files and fonts
* [`libspiro`](https://github.com/fontforge/libspiro) Raph Levien's `clothoid` to Bezier's `spline` conversion routines. If this is available FontForge will allow you to edit with `clothoid splines` (`spiro`).

* [`libiconv`](https://www.gnu.org/software/libiconv/) Only important for systems with no built-in `iconv()`. 
...
To do a better job rasterizing bitmap(s), and to enable the TrueType debugger.

Some of FontForge's commands depend on you compiling `freetype` with the byte code interpreter enabled.

It used to be disabled by default because of some [patents granted to Apple](https://freetype.org/patents.html).
...
* `libintl` Is standard on most UNIXes. It is part of the `fink` package on MacOS. It handles UI localization.

* [`libpython`](https://www.python.org/)

If present when FontForge is compiled, allows the user to execute Python scripts within FontForge (and you can configure FontForge so that FontForge's functionality can be `import`'ed into Python -- that is FontForge both *extends* and *embeds* Python)

* [`libX`](https://x.org/) Normally FontForge depends on the `X11` Windowing system, but if you are just interested in the scripting engines (with no UI User Interface), it may be built on systems without `X` (the `configure` script should figure this out).

* [`libcairo`](https://www.cairographics.org/)
`Cairo` handles drawing anti-aliased splines in the outline glyph view. It is dependent on:  `libfontconfig`, `libXft`, and perhaps other libraries.

* [`libpango`](https://www.pango.org/)
`Pango` draws text for complex scripts. It depends on: `glib-2.0`, `libfontconfig`, `libfreetype`, `libXft`, and perhaps other libraries.

* [`Unifont`](https://savannah.gnu.org/projects/unifont)

It includes glyphs for all Unicode codepoints, and FontForge will use it if it is installed.
